...
/*   Updated: 2023/01/07 19:19:40 by tmarts           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	x_fla(int *point, int x_origin)
{
	int	x_cart;
	int	x_isom;

	x_cart = (point[0]- point[1] * cos(0.6155));
	x_isom = x_cart + x_origin;
	return (x_isom);
}

int	y_fla(int *point, int y_origin)
{
	int	y_cart;
	int	y_isom;

	y_cart = (point[0] + point[1]) * sin(0.6155) - point[2];
